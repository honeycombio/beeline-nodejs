...
exports.parseTraceHeader = req => {
  const { headers } = req;
  const { httpTraceParserHook, traceIdSource } = api.validateOpts();
  let parsed = {};
  ...
  if (httpTraceParserHook) {
    return httpTraceParserHook(req);
  } else {
    const headerKey = (api.honeycomb.TRACE_HTTP_HEADER || traceIdSource).toLowerCase();
    parsed = exports.getTraceContext(traceIdSource, req);
    return _includeSource(parsed, headerKey);
  }
};
...
exports.propagateTraceHeader = () => {
  ...
  return {
    [api.honeycomb.TRACE_HTTP_HEADER]: api.honeycomb.unmarshalTraceContext(context),
  };
};

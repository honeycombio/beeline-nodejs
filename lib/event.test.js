...
test("libhoney default config", () => {
  ...
  expect(libhoney.configuredAPIHost).toBe("https://api.honeycomb.io");
  expect(libhoney.configuredDataset).toBe("nodejs");
  expect(libhoney.configuredWriteKey).toBeUndefined();
  expect(libhoney.configuredUserAgentAddition).toBe("honeycomb-nodejs events");
});
...
test("startRequest starts tracking and creates an initial event, finishRequest sends it", () => {
  ...
  expect(libhoney.sentEvents).toEqual([]);
  ...
  expect(libhoney.sentEvents.length).toBe(1);
  let sent = libhoney.sentEvents[0];
  expect(sent.timestamp).toEqual(new Date(startTime));
  expect(sent.data["meta.type"]).toBe("source");
  expect(sent.data["meta.request_id"]).toBe(requestId);
  expect(sent.data["duration_ms"]).not.toBeUndefined();
});

...
exports.parseTraceHeader = (traceIdSource, req) => {
  // check for a custom parser hook config setting
  const { httpTraceParserHook } = propagation.customHooks();

  if (httpTraceParserHook) {
    // if a custom parser was set at configuration, try to apply that first
    const customParsed = propagation.util.applyCustomParser(req, httpTraceParserHook);
    if (customParsed) {
      return customParsed;
    }
  }
...
exports.propagateTraceHeader = () => {
  ...
  const { httpTracePropagationHook } = propagation.customHooks();

  // try to apply a custom propagation hook first
  const customSerialized = propagation.util.applyCustomPropagation(
    propagationContext,

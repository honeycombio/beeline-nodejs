...
// pull trace header key from @opentelemetry
const { TRACE_PARENT_HEADER } = require("@opentelemetry/core");

// exact strings of expected keys for various trace sources
exports.TRACE_HTTP_HEADER = "X-Honeycomb-Trace";
exports.REQUEST_ID_HTTP_HEADER = "X-Request-ID";
exports.OTEL_HTTP_HEADER = TRACE_PARENT_HEADER;
...
// marshalHoneycombTraceContext takes a honeycomb context object
// and returns a serialized honeycomb trace header
exports.marshalHoneycombTraceContext = marshalHoneycombTraceContext;

function marshalHoneycombTraceContext(context) {
  let traceId = context.id;
  let spanId = context.stack[context.stack.length - 1].payload[schema.TRACE_SPAN_ID];
  let contextToSend = context.traceContext || {};
  let dataset = context.dataset;
...
exports.unmarshalHoneycombTraceContext = unmarshalHoneycombTraceContext;
function unmarshalHoneycombTraceContext(contextStr) {
  ...
  switch (version) {
    case "1":
      return unmarshalHoneycombTraceContextV1(payload);
  }
  debug(`unrecognized trace context version ${version}.  ignoring.`);
  return;
}

function unmarshalHoneycombTraceContextV1(payload) {

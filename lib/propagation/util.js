...
exports.applyCustomParser = applyCustomParser;

function applyCustomParser(req, parserHook) {
  if (!parserHook) {
  ...
  try {
    const customParsed = parserHook(req);
...
exports.applyCustomPropagation = applyCustomPropagation;

function applyCustomPropagation(context, propagationHook) {
  if (!propagationHook) {
    // if no custom parser hook, bail to use defaults
    return;
  }

...
function marshalTraceContextv1(context) {
  // this field indicates execution context, use leg
  if (context.id || context.traceContext) {
    return legacyMarshal(context);
  }

  let { traceId, parentSpanId, dataset, customContext } = context;

  let datasetClause = "";
  if (dataset) {
    datasetClause = `dataset=${encodeURIComponent(dataset)},`;
  }
  let contextToSend = customContext || {};
  if (customContext) {
    contextToSend = Buffer.from(JSON.stringify(customContext)).toString("base64");
  }

  return `${VERSION};trace_id=${traceId},parent_id=${parentSpanId},${datasetClause}context=${Buffer.from(
    JSON.stringify(contextToSend)
  ).toString("base64")}`;
}

function legacyMarshal(context) {

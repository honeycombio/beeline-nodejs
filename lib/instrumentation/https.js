...
let instrumentHTTPS = (https, opts = {}) => {
  shimmer.wrap(https, "get", function (_original) {
    ...
    return function (..._args) {
      let req = https.request.apply(this, _args);
      req.end();
      return req;
    };
  });
  ...
  shimmer.wrap(https, "request", function (original) {
    ...
    return function (..._args) {
      ...
      if (_url) {
        ...
        asyncSpanURL = 'https://' + (combinedOptions.hostname || 'localhost')
      ...
      return api.startAsyncSpan(
        ...
        (span) => {
          ...
          // Build up a list of the args we were passed in to pass through to the https.request
          const returnArgs = [];
          if (_url) {

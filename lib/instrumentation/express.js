...
let instrumentExpress = function(express, opts = {}) {
  let userContext, traceIdSource, parentIdSource, httpTraceParserHook, httpTracePropagationHook;
  let packageVersion = opts.packageVersion;

  if (opts.httpTraceParserHook) {
    if (
      typeof opts.httpTraceParserHook === "string" ||
      typeof opts.httpTraceParserHook === "function"
    ) {
  ...
  if (opts.httpTracePropagationHook) {
    if (
      typeof opts.httpTracePropagationHook === "string" ||
      typeof opts.httpTracePropagationHook === "function"
    ) {
      httpTracePropagationHook = opts.httpTracePropagationHook;
    } else {
      debug(
        "httpTracePropagationHook option must either be an string the hook to use or a function returning a string (serialized header value)"
      );
    }
  }

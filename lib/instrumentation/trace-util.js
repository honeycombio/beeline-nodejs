...
exports.getTraceContext = (traceIdSource, req) => {
  if (typeof traceIdSource === "undefined" || typeof traceIdSource === "string") {
    ...
    console.log(`valueAndHeader is ${valueAndHeader}`);
    if (!valueAndHeader) {
      return {};
    }
    ...
    switch (header) {
      ...
      case api.honeycomb.TRACE_HTTP_HEADER: {
        return _includeSource(api.honeycomb.unmarshalTraceContext(value), api.TRACE_HTTP_HEADER);
      }

      case api.aws.TRACE_HTTP_HEADER: {
        return _includeSource(api.aws.unmarshalTraceContext(value), api.aws.TRACE_HTTP_HEADER);
      }

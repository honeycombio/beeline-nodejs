...
describe("marshaling aws", () => {
  test("version string prefix", () => {
    expect(aws.marshalTraceContext(testContext)).toMatch(
      /^Root=1-([\d]||[a-f]){10}-abcdef123456;Parent=1-([\d]||[a-f]){10}-0102030405;userID=1;errorMsg=failed to sign on;toRetry=true/g
    );
  });
});

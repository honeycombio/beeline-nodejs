  ...
  if (typeof traceIdSource === "undefined" || typeof traceIdSource === "string") {
    let headers =
      typeof traceIdSource === "undefined"
        ? [event.TRACE_HTTP_HEADER, "X-Request-ID", "X-Amzn-Trace-Id"]
        : [traceIdSource];
    ...
    switch (header) {
      case event.TRACE_HTTP_HEADER: {
        let parsed = event.unmarshalTraceContext(value);
        if (!parsed) {
          return {};
        }
...
function wrapNextMiddleware(middleware, debugWrap) {
  ...
  return function(req, res, next) {
    ...
    let boundWrappedNext = event.bindFunctionToTrace(function(...args) {
...
const getMagicMiddleware = ({ userContext, traceIdSource, packageVersion }) => (req, res, next) => {
  ...
  let boundFinisher = event.bindFunctionToTrace((response, context) => {
    if (!context) {
      event._askForIssue(

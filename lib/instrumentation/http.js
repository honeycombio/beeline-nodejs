    ...
    return function(_url, options, cb) {
      ...
      return api.startAsyncSpan(
        ...
        span => {
          ...
          const traceHeader = opts.httpTracePropagationHook(api.getTraceContext());
          console.log(api.getTraceContext());
          console.log(traceHeader);
          /*propagateTraceHeader(opts) {
            if (opts.httpTracePropagationHook) {
              return opts.httpTracePropagationHook(getTraceContext());
            }
            return {
              [honeycomb.TRACE_HTTP_HEADER]: honeycomb.marshalTraceContext(getTraceContext())
            };
          }*/

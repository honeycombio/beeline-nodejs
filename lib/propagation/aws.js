...
function marshalTraceContextv1(context) {
  ...
  // check for aws format, add if not there
  // AWS trace fields should have a 1-1234567890-[id] format
  // [version]-[unix timestamp in seconds]-[id]
  const regex = /^1-([\d]||[a-f]){10}-/g;

  // if either of these ids are not in AWS format, convert them before serializing the header
  // Span.payload.startTime is stored in ms, so convert to seconds
  if (!traceId.match(regex)) {
    const rootSpanTimestamp = Math.floor(util.rootSpan(context).startTime / 1000);
    traceId = `${VERSION}-${rootSpanTimestamp}-${traceId}`;
  }
  if (!spanId.match(regex)) {
    const currentSpanTimestamp = Math.floor(util.currentSpan(context).startTime / 1000);
    spanId = `${VERSION}-${currentSpanTimestamp}-${spanId}`;
  }
...
function unmarshalTraceContext(header = "") {
  ...
  if (self) {
    ...
    if (parent) {
      customContext = Object.assign({}, customContext, { Parent: parent });
    }
  }

...
module.exports = class LibhoneyEventAPI {
  ...
  customParseTraceHeader(req) {
    const { httpTraceParserHook } = this.propagationHooks;
  ...
  // to keep options scope in this object, try to serialize with custom hook here
  // return undefined if no custom hook
  customPropagateTraceHeader(context) {
    const { httpTracePropagationHook } = this.propagationHooks;
  ...
  startTrace(fields, traceId, parentSpanId, dataset, propagatedContext) {
    if (!traceId) {
      debug("no active trace found, starting new trace...");
    }

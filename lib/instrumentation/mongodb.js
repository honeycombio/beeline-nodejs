...
function instrumentMongodb(mongodb, opts = {}) {
        ...
        return api.startAsyncSpan(
          ...
          (span) => {
            ...
            if (callback && typeof callback === "function") {
              let wrapped_cb = api.bindFunctionToTrace(function (error, rv) {
                return callback(error, wrapReturnValue(rv, span, eventName));
              });
            ...
            return wrapReturnValue(rv, span, eventName);
          }
        );
      };
    });
  ...
  function wrapReturnValue(rv, span, eventName) {
    let isPromise = rv && typeof rv.then === "function";
    let isCursor = rv instanceof mongodb.Cursor;
    let successHandler = () => api.finishSpan(span, eventName);
    ...
    if (isPromise) {
      rv.then(successHandler, errorHandler);
      return rv;
    }

    if (isCursor) {
      rv.on("close", successHandler);
      rv.on("error", errorHandler);
      return rv;
    }

    // Not a promise or cursor, could be null or something else.
    // Anyway, this span is done.
    api.finishSpan(span, eventName);
    return rv;
  }

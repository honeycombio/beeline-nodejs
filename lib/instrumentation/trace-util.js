...
// opts passed to this function mainly to support tests that pass configs internally
exports.parseTraceHeader = (req, opts = {}) => {
  const { headers } = req;
  const { httpTraceParserHook, traceIdSource } = api.validateOpts(opts);
  ...
  if (httpTraceParserHook) {
    // pass request to custom hook if provided
    const customParsed = httpTraceParserHook(req) || {};
    // if no trace id or span id is parsed, return empty object
    if (!customParsed.traceId || !customParsed.parentSpanId) {
      return {};
    }
    return customParsed;
  } else {
    // run existing parse util if no custom hook is passed
    return exports.getTraceContext(traceIdSource, req);
  }
};

// fetches current active trace and returns an object containing a key/value pair(s)
// of serialized trace headers for propagation
exports.propagateTraceHeader = () => {
  const propagationContext = api.getPropagationContext();
  const { httpTracePropagationHook } = api.validateOpts();

  if (httpTracePropagationHook) {
    return httpTracePropagationHook(propagationContext);
  }
  return {
    [api.honeycomb.TRACE_HTTP_HEADER]: api.honeycomb.marshalTraceContext(propagationContext),
  };
};

// parse a trace header and return object used to populate args to startTrace
// deprecated: in the next major version this should be replaced by parseTraceHeader

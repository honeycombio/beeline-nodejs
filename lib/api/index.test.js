...
const { honeycomb } = require("../propagation");
...
describe("generate trace id", () => {
  ...
  test("should create a 8-byte hex id", () => {
    const regex = /^(([\da-f]){32})$/;
    expect(rootSpan.payload[schema.TRACE_ID]).toMatch(regex);
  });
  api.finishTrace(rootSpan);
});
...
describe("generate span id", () => {
  ...
  test("should create a 8-byte hex id", () => {
    const regex = /^(([\da-f]){16})$/;
    expect(rootSpan.payload[schema.TRACE_SPAN_ID]).toMatch(regex);
  });
  api.finishTrace(rootSpan);
});
...
test("sub-events can opt to rollup count/duration into request events", () => {
  ...
  let durationMS = honey.transmission.events[0].postData[schema.DURATION_MS];
  ...
  let duration2MS = honey.transmission.events[1].postData[schema.DURATION_MS];
  ...
  let duration3MS = honey.transmission.events[2].postData[schema.DURATION_MS];
...
test("sub-events will get manual tracing fields", () => {
  ...
  let subSpanData = honey.transmission.events[0].postData;
  let rootSpanData = honey.transmission.events[1].postData;
...
describe("custom context", () => {
  test("it should be added to request events", () => {
    ...
    api.addTraceContext({ testKey: "testVal" });

    // finish the request
    api.finishTrace(requestPayload);

    let requestData = honey.transmission.events[0].postData;
  ...
  test("it should be added to subevents sent after it was added", () => {
    ...
    let subEventData = honey.transmission.events[0].postData;
    let subEvent2Data = honey.transmission.events[1].postData;
    let reqEventData = honey.transmission.events[2].postData;
...
test("distributed tracing linkage is correct", () => {
  ...
  let marshaledContext = honeycomb.marshalTraceContext(rootContext);

  // this little bit of code simulates a downstream service.
  tracker.setTracked(undefined);
  let ctx = honeycomb.unmarshalTraceContext(marshaledContext);
  ...
  let subSpanEventData = honey.transmission.events[0].postData;
  let subTraceEventData = honey.transmission.events[1].postData;
  let rootEventData = honey.transmission.events[2].postData;
  ...
  // make sure dataset propagates correctly
  expect(honey.transmission.events[0].dataset).toBe("nodejs");
  expect(honey.transmission.events[1].dataset).toBe("nodejs");
  expect(honey.transmission.events[2].dataset).toBe("nodejs");
...
test("async spans share custom context", () => {
  ...
  api.addTraceContext({ custom: "value" });
  ...
  let asyncData = honey.transmission.events[0].postData;
...
test("traceContext, no prefix on incoming trace context, dataset behaves as expected", () => {
  const honey = api._apiForTesting().honey;
  ...
  const postData = honey.transmission.events[0].postData;

  // no prefix on incoming trace level context
  expect(postData.traceLevel).toBe("field");

  // prefix on api-added custom context
  expect(postData["app.custom"]).toBe("value");

  // trace id and span id gets assigned as expected
  expect(postData["trace.trace_id"]).toBeDefined();
  expect(postData["trace.span_id"]).toBeDefined();

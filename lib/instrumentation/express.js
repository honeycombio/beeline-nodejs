...
const getMagicMiddleware = opts => (req, res, next) => {
  const { userContext, parentIdSource, packageVersion } = opts;

  let spanContext = traceUtil.parseTraceHeader(req, opts);
  // parse incoming trace headers into a span context
  /*if (opts.httpTraceParserHook) {
    spanContext = traceUtil.parseTraceHeader(traceIdSource, req);
    spanContext = opts.httpTraceParserHook(req);
  } else {
    spanContext = traceUtil.getTraceContext(traceIdSource, req);
  }*/

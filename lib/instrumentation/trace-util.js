...
exports.parseTraceHeader = (req, opts = {}) => {
  const { headers } = req;
  console.log("parseTraceHeader");
  const { httpTraceParserHook } = opts;
  let parsed = {};
  ...
  const headerKey = (api.honeycomb.TRACE_HTTP_HEADER || opts.traceIdSource).toLowerCase();

  if (httpTraceParserHook) {
    return opts.httpTraceParserHook(req);
  } else {
    //parsed = api.honeycomb.unmarshalTraceContext(headers[headerKey]);
    parsed = exports.getTraceContext(opts.traceIdSource, req);
